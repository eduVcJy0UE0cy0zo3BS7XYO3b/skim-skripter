     ...
     (cond
      ((not (equal? (and music (music-path music))
       ...
       (cond
        ...
        ((and (not music)
              (music? music*))
         (let ((current-path (get-current-music-path))
               (new-path (music-path music*)))
           (when (or (not current-path)
                     (not (string=? current-path new-path)))
             (stop-current-music!)
             (let ((audio-element (set-current-volume (music-audio music*))))
               (media-play audio-element)
               (set-media-loop! audio-element 1)
               (set-current-music-path! new-path))))
         (scene-update-music current music*))
